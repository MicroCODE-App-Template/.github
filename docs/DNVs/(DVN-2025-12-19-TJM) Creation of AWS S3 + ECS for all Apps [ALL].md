# (DVN-2025-12-19-TJM) Creation of AWS S3 + ECS for all Apps [ALL].md
...
This documents how I created Amazon Web Service (AWS) Storage Level-3 (S3) Buckets and Elastic Container Service (ECS) infrastructure for all of MicroCODE's Web Apps. Apps are hosted on AWS ECS, with email handled by Mailgun (not AWS SES).
...
Web-based storage of app and website accessible files, fixed, uploaded, and generated, plus containerized application hosting on AWS ECS.
My NITxPRO profs recommended AWS S3. Period.
We're using AWS ECS for application hosting to keep everything in AWS ecosystem (except email - Mailgun).
...
This is the **recommended S3 bucket architecture** with **names, types, and intent**, aligned with how AWS expects SaaS apps to scale, plus **ECS task definitions and services** for hosting applications on AWS.
...
#### IAM Policy Example (regattarc-server-role)

**Policy for AWS ECS hosted Regatta-RC application:**
...
**Attach this policy to IAM role:** `regattarc-server-role`

**ECS Task Definition Configuration:**

- IAM role attached to ECS task definition
- No access keys needed (uses IAM role)
- Automatic credential rotation
- Environment variables set in ECS task definition (not in code)
...
exports.handler = async (event) => {
  ...
  const ecs = new ECSClient({ region: "us-east-2" });
  ...
  // Apply to all ECS app IAM roles
  const roles = [
    "mcode-server-role",
    "regattarc-server-role",
    "ladders-server-role",
    "ladders-worker-role",
  ];
  ...
  // Optionally scale ECS services to 0 (stops all tasks)
  const clusterName = process.env.ECS_CLUSTER_NAME || "mcode-cluster";
  const services = [
    "mcode-web-service",
    "regattarc-web-service",
    "ladders-web-service",
    "ladders-worker-service",
  ];
  ...
  for (const serviceName of services) {
    try {
      ...
      console.error(`Failed to scale ${serviceName}:`, err);
    }
  }
  ...
  await sns.send(
    new PublishCommand({
      ...
      Message: `S3 budget exceeded. Access disabled for all ECS applications. ECS services scaled to 0. Manual re-enable required.`,
    })
  );
};
```

**Note:** Since apps are hosted on AWS ECS, we can disable IAM role policies AND scale services to 0 tasks. Apps will fail gracefully when AWS access is denied. SNS notifications can trigger webhooks to Mailgun or other services for email alerts.
...
# AWS Configuration (for ECS hosted apps)
# IAM roles are attached to ECS task definitions - no access keys needed
AWS_REGION=us-east-2

# ECS Configuration
ECS_CLUSTER_NAME=mcode-cluster
ECS_SERVICE_NAME=mcode-web-service  # Set per service

# Note: IAM roles are used instead of access keys for ECS tasks
# Each ECS service uses its own IAM role (mcode-server-role, regattarc-server-role, etc.)
...
- Attached to ECS task definitions
- No access keys needed (uses IAM roles)
- Automatic credential rotation
- More secure than access keys

### 7.2 IAM Policy Examples

See section 6.1 for complete IAM policy JSON examples. Attach these policies to IAM roles (not users) for ECS applications.
...
9. **Code Updates:**
   ...
   - [ ] Attach IAM roles to ECS task definitions
   - [ ] Update all S3 calls in codebase
   - [ ] Test upload/download flows from ECS tasks
   - [ ] Test CloudFront URLs
...
3. **Create IAM roles and policies** (for AWS ECS hosting)
...
3. **Test end-to-end flows** from ECS hosted apps
...
- **Apps hosted on AWS ECS** (containerized applications) - using IAM roles (no access keys)
...
### AWS ECS Cost Management

**Important: "Users" for ECS Cost = Concurrent Active Users**

AWS ECS costs are driven by **concurrent users** (active at the same time), not total registered users. This determines CPU/RAM needs and task sizing.
...
#### ECS Task Sizing Guide

**Based on Peak Concurrent Users:**

| Peak Concurrent Users | Task Size | vCPU | RAM    | Tasks | Est. Cost/Month |
| --------------------- | --------- | ---- | ------ | ----- | --------------- |
| 1-5 users             | Small     | 0.25 | 512 MB | 1-2   | ~$10            |
| 5-15 users            | Medium    | 0.5  | 1 GB   | 1-3   | ~$20            |
| 15-50 users           | Large     | 1    | 2 GB   | 2-4   | ~$40            |
| 50-100 users          | XLarge    | 2    | 4 GB   | 3-6   | ~$80            |
| 100+ users            | 2XLarge   | 4    | 8 GB   | 4-8   | ~$160           |
...
1. **Background Workers:** Reduce web server load → smaller task sizes needed
...
4. **Auto-scaling:** ECS can scale tasks based on CPU/memory → pay only for what you use
5. **IAM Roles:** No access keys to manage → more secure
...
| User Count          | Peak Concurrent | Web Task Size | Workers       | Monthly Cost |
| ------------------- | --------------- | ------------- | ------------- | ------------ |
| Beta (< 50 DAU)     | 10-20           | 1 vCPU, 2 GB  | 2 × 0.25 vCPU | ~$66         |
| Small (100-500 DAU) | 20-50           | 1 vCPU, 2 GB  | 2 × 0.25 vCPU | ~$80         |
| Medium (500-2K DAU) | 50-100          | 2 vCPU, 4 GB  | 2 × 0.5 vCPU  | ~$150        |
| Large (2K+ DAU)     | 100+            | 4 vCPU, 8 GB  | 2 × 1 vCPU    | ~$300        |

**Note:** Costs scale with concurrent load and task count (auto-scaling). Background workers help keep web server costs lower. ECS auto-scaling means you pay for what you use.

---

#### Regional Deployment

**Recommendation:** Deploy ECS services in `us-east-2` (Ohio) to match S3 region

**Benefits:**

- Lower latency to S3 buckets
- Reduced data transfer costs (no cross-region charges)
- Better performance for Detroit/Mid-West users
- All services in same region simplifies management

**Configuration:**

- Create ECS cluster in `us-east-2`
- All services use same cluster
- Set AWS_REGION environment variable to `us-east-2`
- ALB automatically routes traffic to tasks in same region
...
1. **CPU Utilization:**
   ...
   - If consistently > 70% → Consider larger task size or more tasks
   - Monitor in CloudWatch
   - Set alert at 70% threshold
   - Use for auto-scaling triggers

2. **Memory Utilization:**

   - If > 80% of allocated RAM → Consider larger task size
   - Monitor in CloudWatch
...
3. **Task Count:**
   ...
   - Monitor number of running tasks
   - Auto-scaling adjusts based on load
   - Set min/max limits to control costs

4. **Request Rate:**

   - Monitor requests per second
   - Affects ALB data processing costs
   - Use CloudFront to reduce origin requests

5. **Response Time:**
   - If slow (> 500ms average) → May need more CPU/RAM or more tasks
   - Monitor in CloudWatch
   - Use ALB target response time metrics

**Optimization Strategies:**

1. **Right-Size Tasks:**

   - Start with smaller task sizes (0.5 vCPU, 1 GB)
   - Monitor CPU/RAM usage
   - Increase only if consistently > 70% CPU or > 80% RAM

2. **Use Auto-Scaling:**

   - Scale tasks based on CPU/memory utilization
   - Set appropriate min/max limits
   - Pay only for what you use

3. **Combine Workers** (if possible):

   - Current: 2 separate worker services
   - Option: Single worker service running both jobs
   - **Savings:** ~$5/month (1 service instead of 2)

4. **Use CloudFront:**

   - Serves static assets from CDN
   - Reduces ALB data processing costs
...
5. **Optimize Container Images:**
   ...
   - **Savings:** Lower ECR storage costs

6. **CloudWatch Logs Retention:**

   - Set appropriate retention (7-30 days)
   - Filter unnecessary logs
   - **Savings:** Lower CloudWatch costs

7. **Single ALB for Multiple Services:**

   - Use path-based routing or host-based routing
   - **Savings:** One ALB instead of multiple
...
- Want simpler deployment (consider Render.io, Railway)
- Don't need AWS-specific features
- Prefer managed platform-as-a-service
- Want lower initial costs for small scale

**Alternative Options:**

| Platform        | 3 Services (Beta) | Notes                        |
| --------------- | ----------------- | ---------------------------- |
| AWS ECS/Fargate | ~$66/month        | Current choice - AWS native  |
| Render.io       | $29/month         | Simpler, less control        |
| Railway.app     | $20/month         | Similar to Render.io         |
| Fly.io          | $10-20/month      | Global edge, different model |
| Heroku          | $75/month         | More expensive               |

**Why ECS Over Alternatives:**

- **IAM Roles:** More secure than access keys
- **AWS Integration:** Seamless with S3, CloudFront, KMS
- **Cost Control:** Pay only for what you use (auto-scaling)
- **Scalability:** Handles growth better than fixed plans
- **Control:** Fine-grained control over infrastructure

---

#### Integration with AWS S3

**How AWS ECS and AWS S3 Work Together:**
...
1. **ECS Web Service:**
   ...
   - Uses IAM role attached to task definition (no access keys)

2. **ECS Workers:**

   - Process background jobs (onboarding, usage)
   - May generate files and upload to S3
   - Use same IAM role as web service (or separate role)
   - Credentials automatically injected via IAM role
...
- CloudFront reduces ALB data processing costs
- S3 handles storage (cheaper than ECS ephemeral storage)
- ECS handles compute with auto-scaling (pay for what you use)
- All in same AWS account → easier cost tracking

**Environment Variables (ECS Task Definition):**

```bash
# AWS Configuration (set in ECS task definition)
AWS_REGION=us-east-2
...
# Note: IAM role attached to task definition provides AWS credentials
