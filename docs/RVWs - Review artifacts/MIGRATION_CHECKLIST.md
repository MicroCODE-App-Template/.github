...
- ✅ Add `const utility = require('../helper/utility');` to each MongoDB model file
- ✅ Update `id` field in schema: `default: () => utility.unique_id('prefix')`
- ✅ Use correct prefix for each entity (see PHASE3_ID_PREFIXES.md)
- ✅ Test: Verify schemas compile correctly

#### Step 3.3.2: Remove Explicit ID Generation from Create Functions (MongoDB Models)

- ✅ Remove `const { v4: uuidv4 } = require('uuid');` from files (if only used for id)
- ✅ Remove all `data.id = uuidv4();` lines (schema default handles it automatically)
- ✅ Do NOT add explicit generation - rely on schema defaults only
- ✅ Test: All create operations work - IDs generated by schema defaults
...
- ✅ `event.mongo.js`: `time` → `created_at`
- ✅ `feedback.mongo.js`: `date_created` → `created_at`
- ✅ `invite.mongo.js`: `date_sent` → `created_at`
- ✅ `key.mongo.js`: `date_created` → `created_at`
- ✅ `log.mongo.js`: `time` → `created_at`
